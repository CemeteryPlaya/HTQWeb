...
let ngrokEndpointCache = null;
let ngrokEndpointCacheTs = 0;
let ngrokEndpointInflight = null;
...
function getConfiguredNgrokLocalPort() {
    return Number.parseInt(String(process.env.NGROK_LOCAL_TCP_PORT || config.mediasoup.webRtcServerPort), 10);
}
function pickNgrokTcpTunnel(payload, targetPort) {
    const tunnelsRaw = payload &&
        typeof payload === 'object' &&
        Array.isArray(payload.tunnels)
        ? payload.tunnels
        : [];
    const tcpTunnels = tunnelsRaw.filter((tunnel) => {
        const publicUrl = tunnel && typeof tunnel === 'object'
            ? String(tunnel.public_url || '')
            : '';
    ...
    const exact = tcpTunnels.find((tunnel) => {
        const configRecord = tunnel.config && typeof tunnel.config === 'object'
            ? tunnel.config
            : null;
...
async function getNgrokEndpointFromApi() {
    ...
    try {
        ...
        console.warn(`[Server] Failed to resolve ngrok host ${host}; using hostname candidate fallback.`, error);
        return {
            ip: host,
            port,
            host,
...
function getNgrokEndpointFromEnv() {
...
async function getNgrokPublicEndpoint() {
    ...
    ngrokEndpointInflight = (async () => {
        try {
            ...
            if (discovered) {
                ...
                console.log(`[Server] Using ngrok public endpoint (${discovered.source}): ` +
            ...
            if (!ngrokDiscoveryWarned) {
                ...
                console.warn(`[Server] Could not auto-discover ngrok endpoint from ${NGROK_API_URL}. ` +
                    'ICE candidates will not be rewritten unless NGROK_PUBLIC_IP/NGROK_PUBLIC_PORT are set.', error);
            }
            return null;
        }
        finally {
...
async function rewriteIceCandidatesForNgrok(candidates) {
...
function closeMediasoupResources(reason) {
    if (reason) {
    ...
    for (const [worker, webRtcServer] of webRtcServers) {
        try {
            ...
            console.warn(`[Server] Failed to close WebRtcServer for worker ${worker.pid}:`, error);
        }
    }
    webRtcServers.clear();
...
async function createWorkers() {
    ...
    if (requestedWorkers > 1) {
        numWorkers = 1;
        console.warn(`[Server] WEBRTC_SERVER_PORT=${webRtcServerPort} is single-port mode. ` +
    ...
    if (listenInfos.length === 0) {
        throw new Error('[Server] Both WEBRTC_ENABLE_UDP and WEBRTC_ENABLE_TCP are disabled; no listenInfos configured.');
    }
    ...
    for (let i = 0; i < numWorkers; i++) {
        ...
        worker.on('died', (error) => {
            console.error(`[Server] Mediasoup worker ${worker.pid} died:`, error);
        ...
        try {
            ...
            try {
                ...
                console.warn(`[Server] Failed to close worker ${worker.pid} after bind failure:`, closeError);
            }
            throw error;
        }
        webRtcServers.set(worker, webRtcServer);
        workers.push(worker);
        console.log(`[Server] Worker ${i + 1}/${numWorkers} created (PID ${worker.pid}), ` +
...
async function getOrCreateRoom(roomId) {
    ...
    room = await Room.create(worker, webRtcServer, roomId, {
        ...
        onNewConsumerNeeded: async (consumingPeerId, _producerPeerId, producer) => {
            await createConsumerForPeer(roomId, consumingPeerId, producer.id);
        },
    });
 ...
 */
async function createConsumerForPeer(roomId, consumingPeerId, producerId) {
    ...
    try {
        ...
        const consumerData = await room.consume(consumingPeerId, peerConn.recvTransportId, producerId, peerConn.rtpCapabilities);
        if (!consumerData)
            return;
        ...
        console.error(`[Server] Failed to create consumer for ${consumingPeerId}:`, err);
    }
}
/**
 ...
 */
async function syncExistingConsumersForPeer(peerId) {
...
function attachPeerToRoom(peerId, roomId) {
    const peerConn = peerConnections.get(peerId);
    if (!peerConn)
        return;
...
function detachPeerFromRoom(peerId, roomId) {
    if (!roomId)
        return;
    const peers = roomMembers.get(roomId);
    if (!peers)
...
function cleanupPeerConnection(peerId, reason) {
    const peerConn = peerConnections.get(peerId);
...
function isAllowedWsPath(pathname) {
...
function isSecureRequest(req) {
    const isDirectTls = !!req.socket?.encrypted;
    if (isDirectTls) {
        return true;
    }
...
function rejectUpgrade(req, socket, statusCode, reason) {
    ...
    console.warn(`[Server] WS upgrade rejected (${statusCode}): path=${path} origin=${origin} reason=${reason}`);
}
/**
 ...
 *   ws.send(reply);
 */
async function handleMessage(peerId, raw) {
    let msg;
    try {
        msg =
            typeof raw === 'string'
    ...
    try {
        switch (signalMethod) {
            ...
            case 'join_room': {
                ...
                if (!roomId)
                    return respondError('roomId is required');
            ...
            case 'joinRoom': {
                ...
                respond({
                    ...
                    turnConfig: config.turn.urls.length > 0
                        ? {
            ...
            case 'createTransport': {
                const room = rooms.get(peerConn.roomId);
                if (!room)
                    return respondError('Not in a room');
                const data = asRecord(msg.data);
                const transportData = await room.createWebRtcTransport(peerId);
                const iceCandidates = await rewriteIceCandidatesForNgrok(transportData.iceCandidates);
                // Track direction
            ...
            case 'connectTransport': {
                ...
                await room.connectTransport(peerId, transportId, data.dtlsParameters);
            ...
            case 'produce': {
                ...
                if (!room)
                    return respondError('Not in a room');
                const data = asRecord(msg.data);
                const producerId = await room.produce(peerId, String(data.transportId || ''), data.kind, data.rtpParameters);
                await syncExistingConsumersForPeer(peerId);
                respond({ producerId });
                break;
            }
            ...
            case 'ice_candidate': {
                if (!peerConn.roomId)
                    return respondError('Not in a room');
                broadcastToRoom(peerConn.roomId, {
                    method: signalMethod,
                    data: msg.data,
                    fromPeerId: peerId,
                }, peerId);
...
async function main() {
    ...
    if (config.signaling.requireTls && !tlsReady) {
        ...
        throw new Error('SIGNALING_REQUIRE_TLS=true but TLS is not ready: ' +
            missingTlsReasons.join('; ') +
            '. Generate LAN certificate/key and set TLS_CERT/TLS_KEY.');
    }
    if (!tlsReady && (hasCertPath || hasKeyPath)) {
        console.warn('[Server] Partial TLS configuration detected (cert/key mismatch). ' +
    ...
    if (useTls) {
        ...
        console.warn('[Server] Running without TLS. ' +
    ...
    httpServer.on('upgrade', (req, socket, head) => {
        ...
        if (!isWebSocketUpgrade) {
            console.warn(`[Server] Rejecting upgrade — Upgrade: '${req.headers.upgrade}' ` +
    ...
    const wsHeartbeatTimer = setInterval(() => {
        for (const [peerId, peerConn] of peerConnections) {
            ...
            if (!peerConn.isAlive) {
                console.warn(`[Server] Heartbeat timeout for ${peerId}. Terminating stale socket.`);
                ws.terminate();
    ...
    wss.on('connection', (ws, req) => {
        ...
        console.log(`[Server] Client connected: ${peerId} path=${getRequestPath(req)} origin=${req.headers.origin || 'n/a'}`);
        // Send peer their ID
    ...
    httpServer.listen(config.port, config.host, () => {
        ...
        console.log(`[Server] Signaling: ${wsProtocol}://${config.host}:${config.port}${config.signaling.wsPaths[0]}`);
        console.log(`[Server] WS paths: ${config.signaling.wsPaths.join(', ')} | ` +
            `Origin check: ${config.signaling.disableOriginCheck ? 'disabled' : 'enabled'}`);
        if (!config.signaling.disableOriginCheck) {
            console.log(`[Server] Allowed origins: ${config.signaling.allowedOriginPatterns.join(', ')}`);
        }
        console.log(`[Server] Workers: ${workers.length}, ` +
...
function parseQualityReport(input) {
...
main().catch((err) => {
    ...
    const errorMessage = err &&
        ...
        : '';
    const isWebRtcPortInUse = errorMessage.includes('address already in use') ||
    ...
    if (isWebRtcPortInUse) {
        ...
        console.error(`[Server] WEBRTC_SERVER_PORT=${webRtcPort} is already in use. ` +
            'Stop the process holding this port or set a different WEBRTC_SERVER_PORT in sfu/.env.');
        console.error(`[Server] Quick check (Windows): cmd /c netstat -ano | findstr :${webRtcPort}`);
    }
    closeMediasoupResources('fatal startup error');
    const isSpawnPermissionError = err &&
        typeof err === 'object' &&
        err.code === 'EPERM' &&
        err.syscall === 'spawn';
    if (isSpawnPermissionError) {
        console.error('[Server] Mediasoup worker could not be spawned (EPERM). ' +
            'On Windows, prefer running SFU in Docker/WSL2 or allow mediasoup-worker.exe in security policy.');
        console.error('[Server] Check executable: sfu/node_modules/mediasoup/worker/out/Release/mediasoup-worker.exe');
    }
    process.exit(1);
});
