...
function resolveSfuHost() {
    ...
    if (normalized === 'localhost' || normalized === '127.0.0.1' || normalized === '::1') {
        console.warn(`[config] SFU_HOST=${configuredHost} is loopback-only. ` +
...
function parseAllowedOriginPatterns() {
    const raw = envStr('SIGNALING_ALLOWED_ORIGINS', 'https://*.instatunnel.my,http://*.instatunnel.my');
    return parseList(raw);
}
function parseWsPaths() {
...
function parseListenIpEntry(entry) {
 ...
 */
function validateListenIps(listenIps) {
    for (const entry of listenIps) {
        ...
        if (isWildcard && !announcedIp) {
            throw new Error(`[config] FATAL: listenIp "${ip}" is a wildcard but announcedIp is not set. ` +
...
function warnIfSuspiciousBitrate(key, value, minExpectedBps) {
    if (!Number.isFinite(value) || value <= 0) {
        throw new Error(`[config] FATAL: ${key} must be a positive integer bitrate in bps, got "${value}".`);
    }
    if (value < minExpectedBps) {
        const suggestedBps = value * 1000;
        console.warn(`[config] ${key}=${value} looks very low for bps. ` +
...
export const config = {
    ...
    mediasoup: {
        ...
        webRtcTransport: {
            ...
            initialAvailableOutgoingBitrate: envInt('WEBRTC_INITIAL_AVAILABLE_OUTGOING_BITRATE', envInt('VIDEO_BITRATE_BPS', 12_000_000)),
            maxIncomingBitrate: envInt('WEBRTC_MAX_INCOMING_BITRATE', Math.floor(envInt('VIDEO_BITRATE_BPS', 12_000_000) * 1.25)),
        },
    },
    // ── Bitrate Targets ──
    bitrate: {
...
warnIfSuspiciousBitrate('WEBRTC_INITIAL_AVAILABLE_OUTGOING_BITRATE', config.mediasoup.webRtcTransport.initialAvailableOutgoingBitrate, 100_000);
warnIfSuspiciousBitrate('WEBRTC_MAX_INCOMING_BITRATE', config.mediasoup.webRtcTransport.maxIncomingBitrate, 100_000);
...
warnIfSuspiciousBitrate('BWE_DEGRADED_BITRATE_BPS', config.bwe.degradedBitrateBps, 100_000);
