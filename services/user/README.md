...
Microservice for HTQWeb platform — handles **User identity and authentication service**.

## Architecture

```
Nginx (Gateway) → /api/user/* → This Service (FastAPI)
                                              ↓
...
```bash
docker build -t htqweb/user:latest .
docker run -p 8001:8001 --env-file .env htqweb/user:latest
```
...
| GET | `/api/user/` | List items (example) |
| POST | `/api/user/` | Create item (example) |
...
1. Legacy data was copied to this service's schema (`user`)
