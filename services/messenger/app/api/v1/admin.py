...
async def list_all_rooms(
    ...
    result = await session.execute(select(ChatRoom).offset(offset).limit(limit))
...
async def list_messages_in_room(
    ...
    result = await session.execute(select(EncryptedMessage).where(EncryptedMessage.room_id == room_id))
    return result.scalars().all()
