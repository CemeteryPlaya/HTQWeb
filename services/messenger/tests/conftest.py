"""Shared test fixtures for Messenger tests."""
...
async def async_engine(pg_url: str):
    ...
    async with engine.begin() as conn:
        await conn.execute(text("CREATE SCHEMA IF NOT EXISTS messenger"))
        await conn.execute(text("CREATE EXTENSION IF NOT EXISTS ltree"))
        await conn.execute(text("SET search_path TO messenger, public"))
...
async def client(async_engine) -> AsyncGenerator[AsyncClient, None]:
    """Provide an httpx AsyncClient wired to the Messenger FastAPI app."""
