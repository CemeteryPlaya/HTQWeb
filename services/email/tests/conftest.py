"""Shared test fixtures for Email tests."""
...
async def async_engine(pg_url: str):
    ...
    async with engine.begin() as conn:
        await conn.execute(text("CREATE SCHEMA IF NOT EXISTS email"))
        await conn.execute(text("SET search_path TO email, public"))
...
async def client(async_engine) -> AsyncGenerator[AsyncClient, None]:
    """Provide an httpx AsyncClient wired to the Email FastAPI app."""
