...
Microservice for HTQWeb platform — handles **HR management microservice**.

## Architecture

```
Nginx (Gateway) → /api/hr/* → This Service (FastAPI)
                                              ↓
...
```bash
docker build -t htqweb/hr:latest .
docker run -p 8001:8001 --env-file .env htqweb/hr:latest
```
...
| GET | `/api/hr/` | List items (example) |
| POST | `/api/hr/` | Create item (example) |
...
1. Legacy data was copied to this service's schema (`hr`)
